...
describe("libjoynr-js.joynr.capabilities.arbitration.Arbitrator", function() {
    capabilities = [
        {
            domain: "myDomain",
    ...
    function returnCapabilitiesFromDiscovery(providerMustSupportOnChange, discoveryEntries, expected, done) {
        ...
        arbitrator
            .startArbitration({
                ...
                interfaceName: interfaceName,
                discoveryQos: new DiscoveryQos(
                    Util.extend(discoveryQos, { providerMustSupportOnChange: providerMustSupportOnChange })
                ),
                proxyVersion: new Version({ majorVersion: 47, minorVersion: 11 })
            })
            .then(onFulfilledSpy)
    ...
    it("returns filtered capabilities from discovery if discoveryQos.providerMustSupportOnChange is true", function(
        done
    ) {
    ...
    it("rejects with NoCompatibleProviderFoundException including a list of incompatible provider version of latest lookup", function(
        ...
        var secondLookupResult = [discoveryEntryWithMajor47AndMinor0, discoveryEntryWithMajor48AndMinor2];
    ...
    it("timeouts after the given discoveryTimeoutMs on empty results", function(done) {
    ...
    it("timeouts after the given discoveryTimeoutMs when capabilityDiscoveryStub throws exceptions", function(done) {
        var onRejectedSpy = jasmine.createSpy("onRejectedSpy");
    ...
    it("arbitrates correctly static capabilities", function(done) {
        spyOn(discoveryQos, "arbitrationStrategy").and.callFake(function(discoveredCaps) {
    ...
    it("fails if arbitrationStrategy throws an exception", function(done) {
    ...
    it("rejects pending arbitrations when shutting down", function(done) {
        var rejectCalled = false;
        capDiscoverySpy.lookup.and.returnValue(Promise.resolve([]));
        spyOn(discoveryQos, "arbitrationStrategy").and.returnValue([]);
        arbitrator = new Arbitrator(capDiscoverySpy);
