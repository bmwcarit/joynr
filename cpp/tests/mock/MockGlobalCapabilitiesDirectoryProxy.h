...
class MockGlobalCapabilitiesDirectoryProxy : virtual public infrastructure::GlobalCapabilitiesDirectoryProxy {
public:

    MockGlobalCapabilitiesDirectoryProxy(std::weak_ptr<JoynrRuntimeImpl> runtime,
        ...
        GlobalCapabilitiesDirectoryProxyBase(runtime, connectorFactory, "testdomain", MessagingQos(10000)),
        GlobalCapabilitiesDirectorySyncProxy(runtime, connectorFactory, "testdomain", MessagingQos(10000)),
        GlobalCapabilitiesDirectoryAsyncProxy(runtime, connectorFactory, "testdomain", MessagingQos(10000)),
        GlobalCapabilitiesDirectoryProxy(runtime, connectorFactory, "testdomain", MessagingQos(10000))
    {

    }

    std::shared_ptr<Future<void>> addAsync(const GlobalDiscoveryEntry &globalDiscoveryEntry,
        ...
        return addAsyncMock(globalDiscoveryEntry, gbids, onSuccess, onApplicationError, onRuntimeError,
                            std::make_shared<MessagingQos>(qos.get()));
    }
    MOCK_METHOD6(addAsyncMock,std::shared_ptr<Future<void>>(
                     const GlobalDiscoveryEntry& globalDiscoveryEntry,
                     const std::vector<std::string>& gbids,
                     std::function<void()> onSuccess,
                     std::function<void(const DiscoveryError::Enum& errorEnum)>
                             onApplicationError,
    ...
    lookupAsync(const std::vector<std::string>& domains,
                const std::string& interfaceName,
                const std::vector<std::string>& gbids,
                std::function<void(const std::vector<GlobalDiscoveryEntry>& result)>
                        onSuccess,
                std::function<void(const DiscoveryError::Enum& errorEnum)>
                        onApplicationError,
                std::function<void(const JoynrRuntimeException& error)>
                        onRuntimeError,
                boost::optional<MessagingQos> qos) noexcept override {
                    return lookupAsyncMock(domains, interfaceName, gbids, onSuccess, onApplicationError, onRuntimeError,
                                           std::make_shared<MessagingQos>(qos.get()));
    }
    MOCK_METHOD7(lookupAsyncMock,
                std::shared_ptr<Future<std::vector<GlobalDiscoveryEntry>>>(
    ...
    MOCK_METHOD6(lookupAsyncMock,
                std::shared_ptr<Future<GlobalDiscoveryEntry>>(
                                    const std::string& participantId,
                                    const std::vector<std::string>& gbids,
                                    std::function<void(const GlobalDiscoveryEntry& result)>
                                            onSuccess,
                                    std::function<void(const DiscoveryError::Enum& errorEnum)>
                                            onApplicationError,
                                    std::function<void(const JoynrRuntimeException& error)>
     ...
     std::shared_ptr<Future<void>> removeAsync(const std::string& participantId,
                                                      const std::vector<std::string>& gbids,
                                                      std::function<void()> onSuccess,
                                                      std::function<void(const DiscoveryError::Enum& errorEnum)>
                                                              onApplicationError,
     ...
     MOCK_METHOD6(removeAsyncMock, std::shared_ptr<Future<void>>(
                      const std::string& participantId,
                      const std::vector<std::string>& gbids,
                      std::function<void()> onSuccess,
                      std::function<void(const DiscoveryError::Enum& errorEnum)>
     ...
     MOCK_METHOD4(touchAsyncMock, std::shared_ptr<Future<void>>(
