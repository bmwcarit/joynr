...
protected:
    ...
    {
        auto settings = std::make_unique<::joynr::Settings>("test-resources/libjoynrSystemIntegration1.settings");
        Settings::merge(Settings{"test-resources/MqttSystemIntegrationTest1.settings"}, *settings, false);
        return settings;
    }
    ...
    static std::vector<Duration> createDurationSequence(const Duration& maxDuration)
    {
        ...
        for (auto interval = std::max(2u, unsigned(std::floor(maxDuration.count() * (quotient - 1) / (std::pow(quotient, n) - 1))));
             interval <= maxDuration.count(); interval *= quotient) {
