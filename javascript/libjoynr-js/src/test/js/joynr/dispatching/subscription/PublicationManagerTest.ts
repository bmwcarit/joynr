...
describe("libjoynr-js.joynr.dispatching.subscription.PublicationManager", () => {
    ...
    let proxyId: any, providerId: any, publicationManager: PublicationManager, joynrInstanceId: any, dispatcherSpy: any;
    ...
    let persistency: any; // localStorage was renamed to persistency because it's impossible to reassign it because of jslint
    ...
    function sharedTests(beforeTests: Function) {
        beforeEach(async () => {
            await beforeTests();
            await prepareTests();
        });
        ...
        it("is instantiable", () => {
            ...
            expect(() => {
                ...
                new PublicationManager(dispatcherSpy, persistency, joynrInstanceId);
            }).not.toThrow();
        });
        ...
        if (persistency) {
            it("restores a persisted subscription request correctly", () => {
                publicationManager.addPublicationProvider(providerId, provider);
                publicationManager.handleSubscriptionRequest(
                    proxyId,
                    providerId,
                ...
                expect(callbackDispatcher.mock.calls.slice(-1)[0][1].subscriptionId).toBe(
                    intervalSubscriptionRequest.subscriptionId
                );
                expect(callbackDispatcher.mock.calls.slice(-1)[0][1].error).toBeUndefined();
            });

            it("restores a persisted event subscription request correctly", () => {
                publicationManager.addPublicationProvider(providerId, provider);
            ...
            it("restores a persisted multicast subscription request correctly", () => {
                publicationManager.addPublicationProvider(providerId, provider);
                ...
                increaseFakeTime(1);
                expect(callbackDispatcher).toHaveBeenCalled();
                expect(callbackDispatcher.mock.calls.slice(-1)[0][1].subscriptionId).toBe(request.subscriptionId);
                expect(callbackDispatcher.mock.calls.slice(-1)[0][1].error).toBeUndefined();
            });
        }
