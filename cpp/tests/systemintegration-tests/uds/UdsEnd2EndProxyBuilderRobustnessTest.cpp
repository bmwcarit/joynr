...
#include "tests/utils/TestLibJoynrUdsRuntime.h"
...
class UdsEnd2EndProxyBuilderRobustnessTest : public Test
{
public:
    UdsEnd2EndProxyBuilderRobustnessTest()
            : domain("cppUdsEnd2EndProxyBuilderRobustnessTest" + util::createUuid()),
              ...
              discoveryQos()
    {
        ...
        consumerRuntime = std::make_shared<TestLibJoynrUdsRuntime>(std::move(settings));

        settings = std::make_unique<Settings>();
        providerRuntime = std::make_shared<TestLibJoynrUdsRuntime>(std::move(settings));
    ...
    ~UdsEnd2EndProxyBuilderRobustnessTest() override
    {
...
protected:
    ...
    std::shared_ptr<TestLibJoynrUdsRuntime> consumerRuntime;
    std::shared_ptr<TestLibJoynrUdsRuntime> providerRuntime;
    ...
    void checkFinishedArbitrators(std::shared_ptr<ProxyBuilder<vehicle::GpsProxy>> gpsProxyBuilder,
...
private:
    DISALLOW_COPY_AND_ASSIGN(UdsEnd2EndProxyBuilderRobustnessTest);
};

} // namespace joynr

void UdsEnd2EndProxyBuilderRobustnessTest::buildProxyBeforeProviderRegistration(
...
void UdsEnd2EndProxyBuilderRobustnessTest::buildMultipleProxiesUsingSameProxyBuilder()
{
    // prepare provider
    auto mockProvider = std::make_shared<MockGpsProvider>();
    types::ProviderQos providerQos;
    ...
    for (int i = 0; i < 10; i++) {
        gpsProxyBuilder->setDiscoveryQos(discoveryQos)->build();
    }
...
void UdsEnd2EndProxyBuilderRobustnessTest::checkFinishedArbitrators(
        std::shared_ptr<ProxyBuilder<vehicle::GpsProxy>> gpsProxyBuilder,
        size_t expectedNumberOfFinishedArbitrators) const
{
    // the adding to gpsProxyBuilder->_finishedArbitratorIds is async, wait for it a limited time
    std::unique_lock<std::mutex> lock(
            gpsProxyBuilder->_finishedArbitratorIdsMutex, std::defer_lock);
    for (int i = 0; i < 100; i++) {
        lock.lock();
...
void UdsEnd2EndProxyBuilderRobustnessTest::attemptBuildAsyncMultipleProxiesUsingSameProxyBuilder()
{
    auto semaphore = std::make_shared<Semaphore>(0);
    ...
    for (int i = 0; i < numberOfProxyBuilds; i++) {
        ...
        auto onError = [semaphore](const exceptions::DiscoveryException&) { semaphore->notify(); };

        gpsProxyBuilder->setDiscoveryQos(discoveryQos)->buildAsync(onSuccess, onError);
    }
...
void UdsEnd2EndProxyBuilderRobustnessTest::
        attemptBuildAsyncMultipleProxiesUsingSameProxyBuilderAndShutdownRuntime()
{
    auto semaphore = std::make_shared<Semaphore>(0);
    std::shared_ptr<TestLibJoynrUdsRuntime> ownConsumerRuntime;

    auto settings = std::make_unique<Settings>();
    ownConsumerRuntime = std::make_shared<TestLibJoynrUdsRuntime>(std::move(settings));
...
void UdsEnd2EndProxyBuilderRobustnessTest::buildProxiesAndVerifyShutdownRuntimeCleansUpCcRuntime()
{
    // prepare provider
    auto mockProvider = std::make_shared<MockGpsProvider>();
    types::ProviderQos providerQos;
    ...
    // for later checking
    for (int i = 0; i < numberOfProxyBuilds; i++) {
        auto onSuccess = [semaphore, &proxyParticipantIds, &gpsProxies, &proxyLock](
...
TEST_F(UdsEnd2EndProxyBuilderRobustnessTest,
...
TEST_F(UdsEnd2EndProxyBuilderRobustnessTest, ProxyBuilderReclaimsArbitrators)
{
    buildMultipleProxiesUsingSameProxyBuilder();
}

TEST_F(UdsEnd2EndProxyBuilderRobustnessTest, ProxyBuilderAsyncReclaimsArbitrators)
{
    attemptBuildAsyncMultipleProxiesUsingSameProxyBuilder();
}

TEST_F(UdsEnd2EndProxyBuilderRobustnessTest, ProxyBuilderAsyncDoesNotCrashWhenRuntimeIsShutdown)
{
    attemptBuildAsyncMultipleProxiesUsingSameProxyBuilderAndShutdownRuntime();
}

TEST_F(UdsEnd2EndProxyBuilderRobustnessTest, ConsumerRuntimeShutdownCleansRoutingTableOfCC)
{
    buildProxiesAndVerifyShutdownRuntimeCleansUpCcRuntime();
}

class UdsEnd2EndProxyBuild : public UdsEnd2EndProxyBuilderRobustnessTest
{
protected:
    void SetUp() override
    {
        UdsEnd2EndProxyBuilderRobustnessTest::SetUp();
...
TEST_F(UdsEnd2EndProxyBuild, buildProxyWithoutSetMessagingQos)
{
    std::shared_ptr<vehicle::GpsProxy> gpsProxy;
...
TEST_F(UdsEnd2EndProxyBuild, buildProxyWithoutSetDiscoveryQos)
{
...
TEST_F(UdsEnd2EndProxyBuild, buildProxyWithoutSetMessagingQosAndWithoutSetDiscoveryQos)
{
    std::shared_ptr<vehicle::GpsProxy> gpsProxy;
