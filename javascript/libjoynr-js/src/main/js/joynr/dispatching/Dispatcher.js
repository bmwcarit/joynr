    ...
    function Dispatcher(clusterControllerMessagingStub, securityManager) {
        ...
        this.sendRequest =
                function sendRequest(settings) {
                    ...
                    var requestMessage = new JoynrMessage(JoynrMessage.JOYNRMESSAGE_TYPE_REQUEST);
                    requestMessage.payload = JSON.stringify(settings.request);
        ...
        this.sendSubscriptionRequest =
                function sendSubscriptionRequest(settings) {
                    ...
                    var requestMessage =
                            new JoynrMessage(JoynrMessage.JOYNRMESSAGE_TYPE_SUBSCRIPTION_REQUEST);
                    // requestMessage.setHeader("subscriptionID", settings.subscriptionRequest.subscriptionId);
                    requestMessage.payload = JSON.stringify(settings.subscriptionRequest);
        ...
        this.sendSubscriptionStop =
                function sendSubscriptionStop(settings) {
                    ...
                    var message =
                            new JoynrMessage(JoynrMessage.JOYNRMESSAGE_TYPE_SUBSCRIPTION_STOP);
                    message.payload = JSON.stringify(settings.subscriptionStop);
        ...
        function sendReply(settings, reply) {
            ...
            replyMessage.payload = JSON.stringify(reply);
        ...
        this.sendPublication = function sendPublication(settings, publication) {
            ...
            // Reply with the result in a JoynrMessage
            var publicationMessage = new JoynrMessage(JoynrMessage.JOYNRMESSAGE_TYPE_PUBLICATION);
            ...
            publicationMessage.payload = JSON.stringify(publication);
                        ...
                        switch (joynrMessage.type) {
                            ...
                            case JoynrMessage.JOYNRMESSAGE_TYPE_REQUEST:
                                try {
                                    ...
                                    reject(new Error("error handling request: " + errorInRequest));
                                }
                                break;
                            ...
                            case JoynrMessage.JOYNRMESSAGE_TYPE_REPLY:
                                try {
                                    ...
                                    reject(new Error("error handling reply: " + errorInReply));
                                }
                                break;
                            ...
                            case JoynrMessage.JOYNRMESSAGE_TYPE_ONE_WAY:
                                try {
                                    requestReplyManager.handleOneWay({
                                        payload : new OneWay(parsePayload(joynrMessage.payload))
                                    });
                                    resolve();
                                } catch (errorInOneWay) {
                                    // TODO do we have to do any erorr handling on a one way
                                    // other than log it?
                                    log.error("error handling one way: " + errorInOneWay);
                                    reject(new Error("error handling one way: " + errorInOneWay));
                                }
                                break;
                            ...
                            case JoynrMessage.JOYNRMESSAGE_TYPE_SUBSCRIPTION_REPLY:
                                try {
                                    ...
                                    reject(new Error("error handling subscriptionReply: "
                                        + errorInSubscriptionReply));
                                }
                                break;
                            ...
                            case JoynrMessage.JOYNRMESSAGE_TYPE_SUBSCRIPTION_STOP:
                                try {
                                    ...
                                    reject(new Error("error handling subscriptionStop: "
                                        + errorInSubscriptionStop));
                                }
                                break;
