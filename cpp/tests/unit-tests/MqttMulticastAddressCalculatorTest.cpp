...
public:
    ...
    MqttMulticastAddressCalculatorTest() :
        ...
        mqttMulticastAddressCalculator(std::make_shared<MqttMulticastAddressCalculator>(
                mqttMulticastTopicPrefix, availableGbids))
    {
...
TEST_F(MqttMulticastAddressCalculatorTest, compute)
{
    ...
    std::vector<std::shared_ptr<const system::RoutingTypes::MqttAddress>> expectedMqttAddressesVector;
    for(std::uint8_t i = 0; i < availableGbids.size(); i++) {
        expectedMqttAddressesVector.push_back(
                std::make_shared<const system::RoutingTypes::MqttAddress>(
                        availableGbids[i], mqttMulticastTopicPrefix + immutableMessage.getRecipient()));
    }
    ...
    for(std::uint8_t i = 0; i < availableGbids.size(); i++) {
        ...
        auto actualMqttAddress = dynamic_cast<const system::RoutingTypes::MqttAddress*>(address.get());
        EXPECT_EQ(*expectedMqttAddressesVector[i], *actualMqttAddress);
        EXPECT_EQ(expectedMqttAddressesVector[i]->getBrokerUri(), actualMqttAddress->getBrokerUri());
