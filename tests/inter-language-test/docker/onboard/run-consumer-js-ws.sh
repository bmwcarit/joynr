...
function usage() {
    echo "Usage: run-consumer-js-ws.sh"
    echo "    [-d <domain>]"
...
cd $ILT_NODE_HOME
npm run-script startjasmine --inter-language-test:domain=$DOMAIN > $ILT_RESULTS_DIR/consumer-js-$FILE_SUFFIX.log 2>&1
EXIT_CODE=$?
echo $EXIT_CODE > $ILT_RESULTS_DIR/consumer-js-$FILE_SUFFIX.exitcode
