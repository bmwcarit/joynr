...
std::shared_ptr<JoynrRuntime> JoynrRuntime::createRuntimeAsync(
        ...
        std::function<void()> onSuccess,
        std::function<void(const exceptions::JoynrRuntimeException& exception)> onError,
        std::shared_ptr<IKeychain>) noexcept
{
    std::shared_ptr<LibJoynrUdsRuntime> runtimeImpl;

    try {
        runtimeImpl = std::make_shared<LibJoynrUdsRuntime>(
                std::move(settings), std::move(onFatalRuntimeError));
