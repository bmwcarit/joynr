...
#define JOYNR_TEST_NO_THROW(statement, fail)                                                       \
    try {                                                                                          \
        ...
        fail() << "Expected: " #statement " doesn't throw an exception.\n"                         \
                  "  Actual: it throws an "                                                        \
               << e.getTypeName() << " with description \"" << e.getMessage() << "\"";             \
    } catch (std::exception & e) {                                                                 \
        fail() << "Expected: " #statement " doesn't throw an exception.\n"                         \
                  "  Actual: it throws an exception with description \""                           \
               << e.what() << "\"";                                                                \
    } catch (...) {                                                                                \
        fail() << "Expected: " #statement " doesn't throw an exception.\n"                         \
                  "  Actual: it throws.";                                                          \
...
#define JOYNR_EXPECT_NO_THROW(statement) JOYNR_TEST_NO_THROW(statement, ADD_FAILURE)

#define JOYNR_ASSERT_NO_THROW(statement) JOYNR_TEST_NO_THROW(statement, FAIL)
...
inline void compareMutableImmutableMessage(const joynr::MutableMessage& mutableMessage,
                                           const joynr::ImmutableMessage& immutableMessage)
{
    // serialize MutableMessage and compare result with ImmutableMessage
    std::unique_ptr<joynr::ImmutableMessage> mutableMessageSerialized =
            mutableMessage.getImmutableMessage();
    EXPECT_EQ(mutableMessageSerialized->getSerializedMessage(),
              immutableMessage.getSerializedMessage());
}

inline void compareMutableImmutableMessage(
        const joynr::MutableMessage& mutableMessage,
