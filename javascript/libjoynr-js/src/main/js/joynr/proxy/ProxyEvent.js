 ...
 * @private
 *
 * @param {Array} unnamedBroadcastValues an array containing the unnamedBroadcastValues, e.g. [1234, "asdf"]
 * @param {?} unnamedBroadcastValues.array the broadcast value
 * @param {Array} broadcastParameter an array of supported parametes
...
class ProxyEvent {
     ...
     * @param {Object} settings the settings for this broadcast proxy
     * @param {String} settings.fromParticipantId of the proxy itself
     * @param {String} settings.toParticipantId of the provider being addressed
     ...
     * @param {SubscriptionManager} settings.dependencies.subscriptionManager
     * @param {Boolean} settings.selective true if the broadcast is selective
     * @param {String} settings.broadcastName the name of the broadcast as modelled in Franca
     * @param {String[]} settings.broadcastParameter the parameter meta information of the broadcast being subscribed to
     * @param {Object} settings.filterParameters the filter parameters of the broadcast
     * @returns {ProxyEvent} */
     ...
     * @function
     * @param {Object} subscribeParameters the settings object for this function call
     * @param {SubscriptionQos} subscribeParameters.subscriptionQos the subscription quality of service object
     * @param {String} [subscribeParameters.subscriptionId] optional subscriptionId. Used to refresh or
     *            reinstate an existing subscription.
     * @param {String[]} [subscribeParameters.partitions] optional parameter for multicast subscriptions.
     ...
     * @param {Function} subscribeParameters.onReceive this function is called when an event as been
     *            received. method signature: "void onReceive({?}value)"
     * @param {Function} subscribeParameters.onError this function is called when an error occurs with
     *            a subscribed event. method signature: "void onError({Error} error)"
     * @param {Function} subscribeParameters.onSubscribed the callback to inform once the subscription request has
     ...
     * @function
     * @param {Object} unsubscribeParameters the settings object for this function call
     * @param {Object} unsubscribeParameters.subscriptionId the subscription token retrieved from the
     *            subscribe function
     * @returns {Object} returns a promise that is resolved when unsubscribe has been executed.
     * @see ProxyEvent#subscribe
     */
    unsubscribe(unsubscribeParameters) {
