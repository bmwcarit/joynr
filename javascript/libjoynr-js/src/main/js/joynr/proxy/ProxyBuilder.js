...
class ProxyBuilder {
     ...
     * @constructor
     *
     * @param {Object} proxyDependencies injected for the children of the proxyBuilder and its children
     * @param {Arbitrator} proxyDependencies.arbitrator used by the proxyBuilder to find the correct provider
     * @param {RequestReplyManager} proxyDependencies.requestReplyManager passed on to proxyAttribute and
     *            proxyOperation
     * @param {SubscriptionManager} proxyDependencies.subscriptionManager passed on to proxyAttribute and
     *            proxyOperation
     * @param {PublicationManager} proxyDependencies.publicationManager passed on to proxyAttribute and
     *            proxyOperation
     * @param {Object} dependencies injected for the proxyBuilder
     * @param {MessageRouter} dependencies.messageRouter the message router
     * @param {LoggingManager} dependencies.loggingManager provides the logging context
     * @param {Address} dependencies.libjoynrMessagingAddress address to this libjoynr's message receiver
     * @param {TypeRegistry} dependencies.typeRegistry the typeRegistry being able to augment raw objects with
     *            type information
     */
    constructor(proxyDependencies, dependencies) {
         ...
         * @function
         *
         * @param {Function} ProxyConstructor - the constructor function of the generated Proxy that
         *            creates a new proxy instance
         * @param {Object} settings - the settings object that is passed to the constructor when building
         *            the object
         * @param {String} settings.domain - the domain on which the provider should be looked for
         * @param {DiscoveryQos} settings.discoveryQos - the settings object determining arbitration
         *            parameters
         * @param {MessagingQos} settings.messagingQos - the settings object determining messaging quality of
         *            service parameters
         * @param {Boolean} settings.freeze - define if the returned proxy object should be frozen
         * @param {Object} settings.loggingContext - optional logging context will be appended to logging
         ...
         * @throws {Error} if arbitrator was not provided
         */
        this.build = function build(ProxyConstructor, settings) {
            ...
            proxy.proxyParticipantId = nanoid();
                ...
                return dependencies.messageRouter
                    ...
                    .catch(error => {
                        const errorMsg = `Proxy creation for domain ${proxy.domain}, interface ${
                            ...
                            proxy.proxyParticipantId
                        } failed. Exception occurred while registering the address to MessageRouter. Error: ${
                            error.stack
                        }`;
                        log.debug(errorMsg);
