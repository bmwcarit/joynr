...
public class DefaultStatelessAsyncIdCalculatorImpl implements StatelessAsyncIdCalculator {
    ...
    /**
     * When given as input a type 3 UUID as returned by {@link #calculateParticipantId(String, StatelessAsyncCallback)}
     * then this method will return a String value which can be used to uniquely identify the stateless callback which
     * should be used for handling a reply.
     *
     * @param participantIdUuid the type 3 UUID as generated by {@link #calculateParticipantId(String, StatelessAsyncCallback)}.
     * @return an identifier string which allows the runtime to choose the correct stateless async callback to use.
     * @throws io.joynr.exceptions.JoynrIllegalStateException if there is no known stateless async callback associated
     * with the given UUID.
     */
    private String getParticipantIdForUuid(String participantIdUuid) {
        return Optional.ofNullable(participantIdMap.get(participantIdUuid))
                       .orElseThrow(() -> new JoynrIllegalStateException("Unknown stateless participant ID UUID: "
                               + participantIdUuid));
    }
}
