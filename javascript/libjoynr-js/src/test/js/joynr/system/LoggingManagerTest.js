    ...
    describe("libjoynr-js.joynr.util.LoggingManager", function() {
        ...
        it(
                ...
                function(done) {
                    ...
                    var appender = loggingManager.createAppender(config);
                    expect(loggingManager.getAppender(config.name)).toEqual(appender);
                    expect(loggingManager.getAppender(config.name) instanceof MyAppenderType)
                            .toEqual(true);
                    done();
                });
        ...
        it("loads loggers and appenders as defined in a config with arrays", function(done) {
            ...
            //        .toEqual(true);
            expect(loggingManager.getAppender("WEBWORKER") instanceof WebWorkerMessagingAppender)
                    .toEqual(true);
            expect(loggingManager.getLogger("TestLogger1").getLevel()).toEqual(
                    loggingManager.getLogLevel("info"));
            expect(loggingManager.getLogger("TestLogger2").getLevel()).toEqual(
                    loggingManager.getLogLevel("error"));
            done();
        });
