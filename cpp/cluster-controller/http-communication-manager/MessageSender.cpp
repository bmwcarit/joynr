      ...
      channelUrlContactorDelayedScheduler(NULL)
{
    ...
    channelUrlContactorDelayedScheduler = new ThreadPoolDelayedScheduler(
                channelUrlContactorThreadPool,
...
void MessageSender::sendMessage(
    ...
    /**
     * NOTE: passing QString by value into the runnable and thereby relying on the fact that the QString internally
     * uses QSharedDataPointer to manage the string's data, which when copied by value only copies the pointer (but safely)
     */
    scheduler->schedule(
...
void MessageSender::SendMessageRunnable::run() {
    ...
    if(sendMessageResult.getStatusCode() != 201) {
        ...
        LOG_ERROR(logger,
                  QString("sending message - fail; error message %1; contents %2; scheduling for retry...")
                    .arg(sendMessageResult.getErrorMessage())
                    .arg(body)
        );
    } else {
        LOG_DEBUG(logger, "sending message - success; url: " + url + " status code: "
