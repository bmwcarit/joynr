    ...
    public void testGeneratesEnumwithDefaultValue() throws Exception {
        ...
        result.forEach((filename, fileContent) -> {
            if (filename.equals("TestStruct")) {
                testResult.setDefaultEnumInitializationFound(fileContent.contains("testEnumerationType(joynr::types::v1::TestEnumerationType::VALUE1)"));
                testResult.setDefaultEnumInitializationWithExplicitValueFound(fileContent.contains("testEnumerationTypeWithExplicitValue(joynr::types::v1::TestEnumerationTypeWithExplicitValue::VALUEWITHDEFAULT1)"));
            }
        });
