...
joynr_logging::Logger* ChannelUrlSelector::logger = joynr_logging::Logging::getInstance()->getLogger("MSG", "ChannelUrlSelector");
...
QString ChannelUrlSelector::obtainUrl(
    ...
    if( channelUrlDirectory.isNull()) {
        ...
        LOG_DEBUG(logger, "using default url constructed from channelId and BounceproxyUrl instead...");
    ...
    LOG_DEBUG(logger, "obtainUrl: trying to obtain Urls from remote ChannelUrlDirectory for id = " +channelId);
    ...
    if (status.successful()) {
        LOG_DEBUG(logger, "obtainUrl: obtained Urls from remote ChannelUrlDirectory for id = " +channelId);
        ...
        entries.insert(channelId, new ChannelUrlSelectorEntry(
                           urlInformation,
                           punishmentFactor,
        ...
        LOG_DEBUG(logger, "obtainUrl: FAILED to obtain Urls from remote ChannelUrlDirectory for id = " +channelId);
        status.setCode(RequestStatusCode::ERROR);
        url = constructDefaultUrl(channelId);
        return constructUrl(url);
    }
...
void ChannelUrlSelector::feedback(bool success,
        const QString& channelId,
        QString url) {
...
QString ChannelUrlSelector::constructDefaultUrl(
        const QString &channelId) {
    LOG_DEBUG(logger, "constructDefaultUrl ... using default Url inferred from channelId and BounceProxyUrl");
    if (! useDefaultUrl) assert(false);
...
joynr_logging::Logger* ChannelUrlSelectorEntry::logger = joynr_logging::Logging::getInstance()->getLogger("MSG", "ChannelUrlSelectorEntry");
...
void ChannelUrlSelectorEntry::updateFitness() {
    ...
    // Is it time to increase the fitness of all Urls? (counterbalances punishments, forget some history)
    ...
    for(int i=0;i<urls.size();i++) {
        ...
        if(urlFitness > urls.size() - i) { // the fitness of an url of rank eg 2 cannot be hiher than 2
