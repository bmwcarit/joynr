...
if [ $NO_CPP_BUILD ]; then
	...
	execute_in_docker '"echo \"Building and packaging MoCOCrW\" && . /etc/profile && /data/src/docker/joynr-base/scripts/test/cpp-build-MoCOCrW-package.sh 2>&1"'

	execute_in_docker '"echo \"Building and packaging mosquitto\" && . /etc/profile && /data/src/docker/joynr-base/scripts/test/cpp-build-mosquitto-package.sh 2>&1"'

	execute_in_docker '"echo \"Building and packaging smrf\" && . /etc/profile && /data/src/docker/joynr-base/scripts/test/cpp-build-smrf-rpm-package.sh 2>&1"'

	execute_in_docker '"echo \"Building joynr c++\" && . /etc/profile && /data/src/docker/joynr-base/scripts/ci/cpp-clean-build.sh --additionalcmakeargs \"-DUSE_PLATFORM_MUESLI=OFF\" --jobs '"${JOBS}"' --enableclangformatter OFF --buildtests OFF 2>&1"'

	execute_in_docker '"echo \"Packaging joynr c++\" && . /etc/profile && /data/src/docker/joynr-base/scripts/test/cpp-build-rpm-package.sh --rpm-spec tests/system-integration-test/docker/onboard/joynr-without-test.spec 2>&1"'
...
if [ $NO_CPP_TEST_BUILD ]; then
	...
	execute_in_docker '"echo \"Building C++ System Integration Tests\" && . /etc/profile && export JOYNR_INSTALL_DIR=/data/build/joynr && echo \"dir: \$JOYNR_INSTALL_DIR\" && /data/src/docker/joynr-base/scripts/ci/cpp-build-tests.sh system-integration-test --jobs '"${JOBS}"' --clangformatter OFF 2>&1"'
fi
