...
namespace {
    ...
    struct BaseTestRunner {
        ...
        template<typename T = FunctionType,
                 typename U = ControlEntry>
        std::enable_if_t<std::is_same<T, tags::Domain>::value, void> completeInitializationOfExpectedEntry(U& entry) {
        ...
        template<typename T = FunctionType,
                 typename U = ControlEntry>
        std::enable_if_t<std::is_same<T, tags::Interface>::value, void> completeInitializationOfExpectedEntry(U& entry) {
        ...
        template<typename T = FunctionType,
                 typename U = ControlEntry>
        std::enable_if_t<std::is_same<T, tags::Domain>::value, void> completeInitializationOfQueryEntry(U& entry) {
        ...
        template<typename T = FunctionType,
                 typename U = ControlEntry>
        std::enable_if_t<std::is_same<T, tags::Interface>::value, void> completeInitializationOfQueryEntry(U& entry) {
            entry.setDomain("DEFAULT_DOMAIN");
        ...
        template<typename T = ControlEntry>
        std::enable_if_t<std::is_same<T, MasterAccessControlEntry>::value>
        run() {
            ...
            boost::optional<MasterAccessControlEntry> queryResult = this->localDomainAccessStore.getMasterAccessControlEntry(queryValue.getUid(),
                                                                     queryValue.getDomain(),
                                                                     queryValue.getInterfaceName(),
                                                                     queryValue.getOperation());
            EXPECT_EQ(expectedValue, queryResult.get());

            EXPECT_TRUE(this->localDomainAccessStore.removeMasterAccessControlEntry(expectedValue.getUid(),
        ...
        template<typename T = ControlEntry>
        std::enable_if_t<std::is_same<T, OwnerAccessControlEntry>::value>
        run() {
            const T expectedValue = getExpectedValue();
            const T queryValue = getQueryValue();

            this->localDomainAccessStore.updateOwnerAccessControlEntry(expectedValue);
            boost::optional<OwnerAccessControlEntry> queryResult = this->localDomainAccessStore.getOwnerAccessControlEntry(queryValue.getUid(),
                                                                     queryValue.getDomain(),
                                                                     queryValue.getInterfaceName(),
                                                                     queryValue.getOperation());
            ...
            EXPECT_TRUE(this->localDomainAccessStore.removeOwnerAccessControlEntry(expectedValue.getUid(),
                                                                                    expectedValue.getDomain(),
                                                                                    expectedValue.getInterfaceName(),
                                                                                    expectedValue.getOperation()));
        }
        ...
        template<typename T = ControlEntry>
        std::enable_if_t<std::is_same<T, MasterRegistrationControlEntry>::value>
        run() {
            const T expectedValue = getExpectedValue();
            const T queryValue = getQueryValue();

            this->localDomainAccessStore.updateMasterRegistrationControlEntry(expectedValue);
            boost::optional<MasterRegistrationControlEntry> queryResult = this->localDomainAccessStore.getMasterRegistrationControlEntry(queryValue.getUid(),
                                                                                                                                         queryValue.getDomain(),
                                                                                                                                         queryValue.getInterfaceName());
            ASSERT_TRUE(queryResult);
            EXPECT_EQ(expectedValue, queryResult.get());

            EXPECT_TRUE(this->localDomainAccessStore.removeMasterRegistrationControlEntry(expectedValue.getUid(),
                                                                                    expectedValue.getDomain(),
                                                                                    expectedValue.getInterfaceName()));
        }

        template<typename T = ControlEntry>
        std::enable_if_t<std::is_same<T, OwnerRegistrationControlEntry>::value>
        run() {
            const T expectedValue = getExpectedValue();
            const T queryValue = getQueryValue();

            this->localDomainAccessStore.updateOwnerRegistrationControlEntry(expectedValue);
            boost::optional<OwnerRegistrationControlEntry> queryResult = this->localDomainAccessStore.getOwnerRegistrationControlEntry(queryValue.getUid(),
                                                                                                                                        queryValue.getDomain(),
                                                                                                                                        queryValue.getInterfaceName());
            ASSERT_TRUE(queryResult);
            EXPECT_EQ(expectedValue, queryResult.get());

            EXPECT_TRUE(this->localDomainAccessStore.removeOwnerRegistrationControlEntry(expectedValue.getUid(),
                                                                                    expectedValue.getDomain(),
                                                                                    expectedValue.getInterfaceName()));
        }
    ...
    template <typename FunctionType, typename ControlEntry>
    struct TestRunner<tags::WildcardMatch, FunctionType, ControlEntry> : public BaseTestRunner<FunctionType, ControlEntry>
    {
    };

    template <typename FunctionType, typename ControlEntry>
    struct TestRunner<tags::ExactMatch, FunctionType, ControlEntry> : public BaseTestRunner<FunctionType, ControlEntry>
    {
    ...
    template <typename FunctionType, typename ControlEntry>
    struct TestRunner<tags::OnlyWildcardMatch, FunctionType, ControlEntry> : public BaseTestRunner<FunctionType, ControlEntry>
    {
...
using matchingTypes = boost::mpl::vector<tags::ExactMatch, tags::WildcardMatch, tags::OnlyWildcardMatch>;
using controlEntries = boost::mpl::vector<MasterAccessControlEntry, OwnerAccessControlEntry, MasterRegistrationControlEntry, OwnerRegistrationControlEntry>;

// Generate all permutations
using TagTypeMplVector = muesli::detail::FlatCartesianTypeProduct<matchingTypes, setFunctions, MakePair>;
using AllCombinations = muesli::detail::FlatCartesianTypeProduct<TagTypeMplVector, controlEntries, MakePair>;
