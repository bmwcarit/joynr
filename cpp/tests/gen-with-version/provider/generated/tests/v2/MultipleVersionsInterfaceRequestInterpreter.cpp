...
#include "joynr/tests/v2/MultipleVersionsInterfaceRequestInterpreter.h"
...
void MultipleVersionsInterfaceRequestInterpreter::execute(
	...
	if (methodName == "getVersionedStruct" && paramTypes.size() == 1
		&& paramTypes.at(0) == "joynr.tests.v2.MultipleVersionsTypeCollection.VersionedStruct"
	) {
		auto requestCallerOnSuccess =
				[onSuccess = std::move(onSuccess)](const joynr::tests::v2::MultipleVersionsTypeCollection::VersionedStruct& result){
					BaseReply reply;
					reply.setResponse(
					result
					);
		...
		joynr::tests::v2::MultipleVersionsTypeCollection::VersionedStruct input;
		try {
	...
	if (methodName == "getAnonymousVersionedStruct" && paramTypes.size() == 1
		&& paramTypes.at(0) == "joynr.tests.v2.AnonymousVersionedStruct"
	) {
		auto requestCallerOnSuccess =
				[onSuccess = std::move(onSuccess)](const joynr::tests::v2::AnonymousVersionedStruct& result){
					BaseReply reply;
					reply.setResponse(
					result
					);
		...
		joynr::tests::v2::AnonymousVersionedStruct input;
		try {
			request.getParams(input);
	...
	if (methodName == "getInterfaceVersionedStruct" && paramTypes.size() == 1
		&& paramTypes.at(0) == "joynr.tests.v2.InterfaceVersionedStruct"
	) {
		auto requestCallerOnSuccess =
				[onSuccess = std::move(onSuccess)](const joynr::tests::v2::InterfaceVersionedStruct& result){
					BaseReply reply;
					reply.setResponse(
					result
					);
		...
		joynr::tests::v2::InterfaceVersionedStruct input;
		try {
			request.getParams(input);
