...
class CapabilitiesRegistrar {
     ...
     * @param {CapabilityDiscovery} dependencies.discoveryStub connects the inProcessStub to its skeleton
     * @param {MessageRouter} dependencies.messageRouter used to register nextHop for registered provider
     * @param {ParticipantIdStorage} dependencies.participantIdStorage connects the inProcessStub to its skeleton
     * @param {Address} dependencies.libjoynrMessagingAddress address to be used by the cluster controller to send incoming requests to this
     *            libjoynr's providers
     * @param {RequestReplyManager} dependencies.requestReplyManager passed on to providerAttribute, providerOperation and providerEvent
     * @param {PublicationManager} dependencies.publicationManager passed on to providerAttribute
     *
     */
     ...
     * @param {Object} settings.provider
     * @param {String} settings.provider.interfaceName
     * @param {ProviderQos} settings.providerQos the Quality of Service parameters for provider registration
     * @param {Number} [settings.expiryDateMs] date in millis since epoch after which the discovery entry can be purged from all directories.
     *            Default value is one day.
     * @param {Object} [settings.loggingContext] optional logging context will be appended to logging messages created in the name of this proxy
     * @param {String} [settings.participantId] optional. If not set, a globally unique UUID participantId will be generated, and persisted to
     ...
     * @param {Boolean} [settings.awaitGlobalRegistration] optional. If provided and set to true registerProvider will wait until local and global
     ...
     * @param {ProviderQos} providerQos the Quality of Service parameters for provider registration
     * @param {Number} [expiryDateMs] date in millis since epoch after which the discovery entry can be purged from all directories.
     *            Default value is one day.
     * @param {Object} [loggingContext] optional logging context will be appended to logging messages created in the name of this proxy
     * @param {String} [participantId] optional. If not set, a globally unique UUID participantId will be generated, and persisted to localStorage.
     * @param {Boolean} [awaitGlobalRegistration] optional. If provided and set to true registerProvider will wait until local and global
     *            registration succeeds or timeout is reached: otherwise registerProvider only waits for local registration.
     *
     * @returns {Object} an A+ promise
     */
     ...
     * @returns {Object} an A+ promise
     */
    async unregisterProvider(domain, provider) {
        this._checkIfReady();
