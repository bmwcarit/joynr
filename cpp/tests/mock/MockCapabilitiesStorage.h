...
public:
    ...
    // from BaseStorage
    boost::optional<DiscoveryEntry> lookupByParticipantId(
            const std::string& participantId) const override
    {
        lookupByParticipantIdMock(participantId);
        return _result;
    }
    MOCK_CONST_METHOD2(lookupByDomainAndInterface,
                       std::vector<DiscoveryEntry>(const std::string& domain,
    ...
    MOCK_METHOD2(insertMock,
                 void(const DiscoveryEntry& entry, const std::vector<std::string>& gbids));
...
public:
    ...
    MOCK_CONST_METHOD2(lookupCacheByParticipantIdMock,
                       void(const std::string& participantId, std::chrono::milliseconds maxAge));
    MOCK_CONST_METHOD3(lookupCacheByDomainAndInterface,
                       std::vector<DiscoveryEntry>(const std::string& domain,
                                                   const std::string& interface,
