...
if [ $NO_CPP_BUILD ]; then
	...
	execute_in_docker '"echo \"Packaging joynr c++\" && . /etc/profile && /data/src/docker/joynr-cpp-base/scripts/build/cpp-build-rpm-package.sh --rpm-spec tests/inter-language-test/docker/onboard/joynr-without-test.spec 2>&1"'
...
if [ $NO_CPP_TEST_BUILD ]; then
	...
	execute_in_docker '"echo \"Building C++ System Integration Tests\" && . /etc/profile && export JOYNR_INSTALL_DIR=/data/build/joynr && echo \"dir: \$JOYNR_INSTALL_DIR\" && /data/src/docker/joynr-cpp-base/scripts/build/cpp-build-tests.sh inter-language-test --jobs '"${JOBS}"' --clangformatter OFF 2>&1"'
fi
