...
public:
    RadioStationListener()
        : logger(Logging::getInstance()->getLogger("DEMO", "MyRadioConsumerApplication::RadioStationListener"))
    {}
...
{
    Logger* logger = Logging::getInstance()->getLogger("DEMO", "MyRadioConsumerApplication::runSubscription");

    // Set the Quality of Service parameters for the subscription

    // The provider will send a notification whenever the value changes. The number of sent notifications
    // may be limited by the min interval QoS.
    // NOTE: The provider must support on-change notifications in order to use this feature by calling
    //       the <attribute>Changed method of the <interface>AbstractProvider class whenever the <attribute>
    //       value changes.
    QSharedPointer<OnChangeWithKeepAliveSubscriptionQos> subscriptionQos(new OnChangeWithKeepAliveSubscriptionQos());
