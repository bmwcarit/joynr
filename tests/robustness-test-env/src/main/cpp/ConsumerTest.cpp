...
class MyBroadcastListener : public joynr::SubscriptionListener<joynr::tests::robustness::MySubscriptionContainer>
{
public:
    ...
    void onSubscribed(const std::string& subscriptionId)
    {
        ...
        JOYNR_LOG_INFO(logger(), "BROADCAST SUBSCRIPTION successful, subscriptionId: {}", subscriptionId);
        JOYNR_LOG_INFO(logger(), "--------------------------------------------------");
    }
...
void ConsumerTest::init()
{
    ...
    for (int i = 0; i < testParams.numOfRuntimes; i++) {
        ...
        runTimeContainerTmp.runtime =
                joynr::JoynrRuntime::createRuntime(testParams.pathToLibJoynrSettings, testParams.pathToMessagingSettings);
    ...
    for (int iRunTimeCount = 0; iRunTimeCount < testParams.numOfRuntimes; iRunTimeCount++) {
        for (int iProxyBuilder = 0; iProxyBuilder < testParams.numOfProxyBuilders; iProxyBuilder++) {
            proxyBuilderContainer proxyBuilderContainerTmp;
            proxyBuilderContainerTmp.proxyBuilder =
                    myRunTimeContainerList[iRunTimeCount].runtime->createProxyBuilder<joynr::tests::robustness::TestInterfaceProxy>(
                            testParams.providerDomain);
            myRunTimeContainerList[iRunTimeCount].proxyBuilderContainerList.emplace_back(std::move(proxyBuilderContainerTmp));
        }
    }
    ...
    for (int iRunTimeCount = 0; iRunTimeCount < testParams.numOfRuntimes; iRunTimeCount++) {
        for (int iProxyBuilder = 0; iProxyBuilder < testParams.numOfProxyBuilders; iProxyBuilder++) {
            for (int iProxy = 0; iProxy < testParams.numOfProxies; iProxy++) {
                ...
                proxyContainerTmp.proxy = myRunTimeContainerList[iRunTimeCount]
                                                  .proxyBuilderContainerList[iProxyBuilder]
                                                  .proxyBuilder->setMessagingQos(joynr::MessagingQos(qosMsgTtl))
                                                  ->setDiscoveryQos(discoveryQos)
                                                  ->build();
                proxyContainerTmp.myProxy =
                        std::make_unique<ConsumerProxy>(*(proxyContainerTmp.proxy), testParams.threadDelayMS);

                // subscribe to broadcaster 1
                auto myBroadcastSubscriptionQos1 = std::make_shared<joynr::MulticastSubscriptionQos>();
                myBroadcastSubscriptionQos1->setValidityMs(validityOfBroadcastSubscriptionDurationMs);
                ...
                myBroadcastSubscriptionIdFuture1 = proxyContainerTmp.proxy->subscribeToBroadcastWithClassParameter1Broadcast(
                ...
                auto myBroadcastSubscriptionQos2 = std::make_shared<joynr::MulticastSubscriptionQos>();
                myBroadcastSubscriptionQos2->setValidityMs(validityOfBroadcastSubscriptionDurationMs);
                ...
                myBroadcastSubscriptionIdFuture2 = proxyContainerTmp.proxy->subscribeToBroadcastWithClassParameter2Broadcast(
                ...
                auto mySelectiveBroadcastSubscriptionQos1 = std::make_shared<joynr::OnChangeSubscriptionQos>();
                mySelectiveBroadcastSubscriptionQos1->setMinIntervalMs(setMinimumIntervalSelectiveBroadcastMs);
                mySelectiveBroadcastSubscriptionQos1->setValidityMs(validityOfSelectiveBroadcastSubscriptionDurationMs);
                ...
                std::shared_ptr<joynr::ISubscriptionListener<std::string>> mySelectiveBroadcastListener1;
                std::shared_ptr<joynr::Future<std::string>> mySelectiveBroadcastSubscriptionIdFuture1;
                mySelectiveBroadcastSubscriptionIdFuture1 =
                        proxyContainerTmp.proxy->subscribeToBroadcastSelectiveWithSingleStringParameter1Broadcast(
                                *mySelectiveBroadcastFilterParams1, mySelectiveBroadcastListener1,
                                mySelectiveBroadcastSubscriptionQos1);
                mySelectiveBroadcastSubscriptionIdFuture1->get(
                        subscriptionIdFutureDefault, proxyContainerTmp.mySelectiveBroadcastSubscriptionId1);

                // subscribe to selective broadcaster 2
                auto mySelectiveBroadcastSubscriptionQos2 = std::make_shared<joynr::OnChangeSubscriptionQos>();
                mySelectiveBroadcastSubscriptionQos2->setMinIntervalMs(setMinimumIntervalSelectiveBroadcastMs);
                mySelectiveBroadcastSubscriptionQos2->setValidityMs(validityOfSelectiveBroadcastSubscriptionDurationMs);
                ...
                std::shared_ptr<joynr::ISubscriptionListener<std::string>> mySelectiveBroadcastListener2;
                std::shared_ptr<joynr::Future<std::string>> mySelectiveBroadcastSubscriptionIdFuture2;
                mySelectiveBroadcastSubscriptionIdFuture2 =
                        proxyContainerTmp.proxy->subscribeToBroadcastSelectiveWithSingleStringParameter2Broadcast(
                                *mySelectiveBroadcastFilterParams2, mySelectiveBroadcastListener2,
                                mySelectiveBroadcastSubscriptionQos2);
                mySelectiveBroadcastSubscriptionIdFuture1->get(
                        subscriptionIdFutureDefault, proxyContainerTmp.mySelectiveBroadcastSubscriptionId2);

                // save
                myRunTimeContainerList[iRunTimeCount].proxyBuilderContainerList[iProxyBuilder].proxyContainerList.emplace_back(
                        std::move(proxyContainerTmp));
            }
        }
    }
    ...
    for (auto& it_runtime : myRunTimeContainerList) {
        for (auto& it_proxyBuilder : it_runtime.proxyBuilderContainerList) {
            for (auto& it_proxy : it_proxyBuilder.proxyContainerList) {
                ...
                try {
                    it_proxy.proxy->unsubscribeFromBroadcastWithClassParameter1Broadcast(it_proxy.myBroadcastSubscriptionId1);
                } catch (const joynr::exceptions::JoynrRuntimeException& e) {
                    JOYNR_LOG_ERROR(logger(), "UNSUBSCRIBE from myBroadcastSubscription 1 FAILED: {}", e.getMessage());
                }

                // unsubscribe from Broadcast2
                try {
                    it_proxy.proxy->unsubscribeFromBroadcastWithClassParameter2Broadcast(it_proxy.myBroadcastSubscriptionId2);
                } catch (const joynr::exceptions::JoynrRuntimeException& e) {
                    JOYNR_LOG_ERROR(logger(), "UNSUBSCRIBE from myBroadcastSubscription 2 FAILED: {}", e.getMessage());
                }

                // unsubscribe from selective broadcast 1
                try {
                    it_proxy.proxy->unsubscribeFromBroadcastSelectiveWithSingleStringParameter1Broadcast(
                            it_proxy.mySelectiveBroadcastSubscriptionId1);
                } catch (const joynr::exceptions::JoynrRuntimeException& e) {
                    JOYNR_LOG_ERROR(
                            logger(), "UNSUBSCRIBE from mySelectiveBroadcastSubscriptionId 1 FAILED: {}", e.getMessage());
                }

                // unsubscribe from selective broadcast 2
                try {
                    it_proxy.proxy->unsubscribeFromBroadcastSelectiveWithSingleStringParameter2Broadcast(
                            it_proxy.mySelectiveBroadcastSubscriptionId2);
                } catch (const joynr::exceptions::JoynrRuntimeException& e) {
                    JOYNR_LOG_ERROR(
                            logger(), "UNSUBSCRIBE from mySelectiveBroadcastSubscriptionId 2 FAILED: {}", e.getMessage());
                }
            }
        }
    }
...
void ConsumerTest::destroyAllContainers()
{
    for (auto& it_runtime : myRunTimeContainerList) {
        for (auto& it_proxyBuilder : it_runtime.proxyBuilderContainerList) {
            ...
            it_proxyBuilder.proxyContainerList.erase(
                    it_proxyBuilder.proxyContainerList.begin(), it_proxyBuilder.proxyContainerList.end());
        }
        it_runtime.proxyBuilderContainerList.erase(
                it_runtime.proxyBuilderContainerList.begin(), it_runtime.proxyBuilderContainerList.end());
    }
