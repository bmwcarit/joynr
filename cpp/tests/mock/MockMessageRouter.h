...
{
public:
    void invokeAddNextHopOnSuccessFct(
    ...
    void invokeRemoveNextHopOnSuccessFct(
            const std::string& /*participantId*/,
            std::function<void()> onSuccess,
            std::function<void(const joynr::exceptions::ProviderRuntimeException&)> /*onError*/)
    {
    ...
    MockMessageRouter(boost::asio::io_service& ioService)
    {
        ...
        using ::testing::_;
        ON_CALL(*this, addNextHop(_, _, _, _, _, _, _))
                .WillByDefault(
                        testing::Invoke(this, &MockMessageRouter::invokeAddNextHopOnSuccessFct));
        ON_CALL(*this, removeNextHop(_, _, _))
                .WillByDefault(
                        testing::Invoke(this, &MockMessageRouter::invokeRemoveNextHopOnSuccessFct));
    }

    MOCK_METHOD2(route,
                 void(std::shared_ptr<joynr::ImmutableMessage> message, std::uint32_t tryCount));

    MOCK_METHOD1(publishToGlobal, bool(const joynr::ImmutableMessage& message));

    MOCK_METHOD6(
            registerMulticastReceiver,
            void(const std::string& multicastId,
    ...
    MOCK_METHOD1(sendQueuedMessages,
                 void(std::shared_ptr<const joynr::system::RoutingTypes::Address> address));
