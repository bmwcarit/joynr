...
function LoggingManager() {
     ...
     */
    this.createAppender = function createAppender(config) {
        var AppenderConstructor, appender, layout;

        AppenderConstructor = appenderTypes[config.type];
        if (AppenderConstructor === undefined) {
            throw new Error("unknown constructor for appender type " + config.type);
        }

        appender = new AppenderConstructor(config, loggingContexts);
        if (config.ThresholdFilter !== undefined && config.ThresholdFilter.level !== undefined) {
            appender.setThreshold(this.getLogLevel(config.ThresholdFilter.level));
        }

        if (config.PatternLayout !== undefined && config.PatternLayout.pattern !== undefined) {
            layout = new log4javascript.PatternLayout(config.PatternLayout.pattern);
     ...
     *            appender
     */
    this.createLogger = function createLogger(config) {
     ...
     *            appender type
     */
    this.registerAppenderClass = function registerAppenderClass(appenderTypeName, constructor) {
