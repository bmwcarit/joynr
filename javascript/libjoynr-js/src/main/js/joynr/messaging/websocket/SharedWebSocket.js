...
/**
 * @param address
 * @param {WebSocketAddress} address to which messages are sent on the clustercontroller.
...
class SharedWebSocket {
     ...
     * @param {WebSocketClientAddress} settings.localAddress address used by the websocket server to contact this
     ...
     * @param {WebSocketAddress} settings.remoteAddress to which messages are sent on the websocket server.
    ...
    constructor(settings) {
        ...
        /**
         * @param {WebSocket} websocket to use for the initialization
         * @param {String} localAddress address used by the cluster controller to contact this
         *            libjoynr
         */
        ...
        /**
         * @param {WebSocket} websocket to use for sending messages
         * @param {Array} queuedMessages
         */
        ...
        function onClose(event) {
            ...
            if (event.code !== SharedWebSocket.EVENT_CODE_SHUTDOWN) {
                log.info(
                    `connection closed unexpectedly. code: ${event.code} reason: ${
                        event.reason
                    }. Trying to reconnect...`
                );
                if (reconnectTimer !== undefined) {
        ...
        Object.defineProperty(this, "onmessage", {
            set(newCallback) {
                if (typeof newCallback === "function") {
                    ...
                    throw new Error(
                        `onmessage callback must be a function, but instead was of type ${typeof newCallback}`
                    );
                }
            },
            get() {
